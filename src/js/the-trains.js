 ...
 * Adapted for UK Rail Viz: data comes from data/network.json +
 * data/marey-index.json via the uk-data adapter; trips are loaded per day
 * (data/marey-trips-<date>-<line>.json) and a day picker re-renders the
 * section; lined-up Marey terminals and end positions are computed from the
 * network instead of hardcoded MBTA stations.
 *
 ...
 * 6. Day picker: load a day's trips and re-render the whole section
 *
...
VIZ.requiresData([
...
}).done(function (networkRaw, mareyIndex) {
  "use strict";

  // the network is converted into the exemplar's station-network / spider /
  // marey-header shapes by the uk-data adapter; the Marey chart consumes
  // header[key][0] (a stop's order along its line) for station x-positions
  var converted = VIZ.ukData.convertNetwork(networkRaw);
  var network = converted.network;
  var spider = converted.spider;
  var header = converted.header;
  var paths = converted.paths;
  var lines = converted.lines;

  // day/line index used to find the per-day marey trip files; the day picker
  // defaults to the last day in the index
  var days = mareyIndex.days || [];
  var defaultDate = days.length ? days[days.length - 1].date : null;
  ...
  // trips are loaded per day below (loadTripsFor) and pre-processed there
  var trips = null;
  var minUnixSeconds;
  var maxUnixSeconds;
  var linedUpTrips;
  ...
  function renderSideMap(mapGlyphContainer, outerWidth, outerHeight) {
    ...
    // terminals are the nodes that sit on only one link
    network.nodes.forEach(function (node) {
      if (node.links && node.links.length === 1) {
        _.unique(node.links.map(function (link) { return link.line; })).forEach(function (line) {
          dot(node.id, line);
        });
      }
    });
  ...
  function renderTrainsAtTime(unixSeconds) {
    ...
    trains
        ...
        .attr('cy', function (d) { return d.pos[1]; });
    trains.exit().remove();
    timeDisplay.text(moment(unixSeconds * 1000).zone(VIZ.ukData.ZONE_OFFSET).format('h:mm a'));
  }
  ...
  //
  // The starting stations are the terminals of the network: the first and
  // last stops of each line's path.  A station may be a terminal for
  // several lines (every line in the current placeholder data serves the
  // full station set), so each terminal only gets one column, positioned
  // at the outermost block edge it touches.
  var linedUpMareyStartingStationLabels = {};
  var linedUpMareyStartingStations = [];
  ...
  var linedUpMareyLineEndPositions = [];
  var linedUpMareyMidpointLabelPositions = [];
  (function calcPlacement() {
    var stopCounts = paths.map(function (path) { return path.length; });
    var totalStops = d3.sum(stopCounts);
    var forLines = 1 - betweenEnds * stopCounts.length - betweenStarts * (stopCounts.length - 1);
    // each line occupies a block proportional to its share of the stops;
    // its start terminal column sits at the block's left edge and its end
    // terminal column at the block's right edge
    var startPosition = {};
    var endPosition = {};
    var blockStart = 0;
    stopCounts.forEach(function (stops, lineIndex) {
      var blockWidth = forLines * stops / totalStops;
      var blockEnd = blockStart + blockWidth + betweenEnds;
      var startId = paths[lineIndex][0];
      var endId = paths[lineIndex][stops - 1];
      startPosition[startId] = (startPosition[startId] === undefined) ? blockStart : Math.min(startPosition[startId], blockStart);
      endPosition[endId] = (endPosition[endId] === undefined) ? blockEnd : Math.max(endPosition[endId], blockEnd);
      blockStart = blockEnd + betweenStarts;
    });
    var seen = {};
    stopCounts.forEach(function (stops, lineIndex) {
      var startId = paths[lineIndex][0];
      var endId = paths[lineIndex][stops - 1];
      // a column's label sits between its line's start and end columns
      var midpoint = d3.mean([startPosition[startId], endPosition[endId]]);
      if (!seen[startId]) {
        seen[startId] = true;
        linedUpMareyStartingStationLabels[startId] = {text: idToNode[startId].name, anchor: 'start'};
        linedUpMareyStartingStations.push(startId);
        linedUpMareyLineEndPositions.push(startPosition[startId]);
        linedUpMareyMidpointLabelPositions.push(midpoint);
      }
      if (!seen[endId]) {
        seen[endId] = true;
        linedUpMareyStartingStationLabels[endId] = {text: idToNode[endId].name, anchor: 'end'};
        linedUpMareyStartingStations.push(endId);
        linedUpMareyLineEndPositions.push(endPosition[endId]);
        linedUpMareyMidpointLabelPositions.push(midpoint);
      }
    });
  }());
  ...
  // lined-up trip data and scales are set per day once the trips load
  ...
  function renderMarey(outerSvg, fullMareyOuterWidth) {
    ...
    var yAxis = d3.svg.axis()
      .tickFormat(function (d) { return moment(d).zone(VIZ.ukData.ZONE_OFFSET).format("h:mm A"); })
    ...
    function select(time) {
      ...
      timeDisplay.text(moment(time * 1000).zone(VIZ.ukData.ZONE_OFFSET).format('h:mm a'));
    ...
    connections
        .attr('d', function (connection) {
          ...
          var annotationY = yScale(moment(connection.parent.time + ' +0100', 'YYYY/MM/DD HH:m ZZ').valueOf() / 1000) - 4;
          var connectionStartY = yScale(moment(connection.start + ' +0100', 'YYYY/MM/DD HH:m ZZ').valueOf() / 1000);
          var connectionEndY = yScale(moment(connection.stop + ' +0100', 'YYYY/MM/DD HH:m ZZ').valueOf() / 1000);
          var connectionSingleY = yScale(moment(connection.time + ' +0100', 'YYYY/MM/DD HH:m ZZ').valueOf() / 1000);
    ...
    annotationContainer.selectAll('text, text tspan')
        ...
        .attr('y', function (d) { return yScale(moment(d.time + ' +0100', 'YYYY/MM/DD HH:m ZZ').valueOf() / 1000); });
    ...
    function highlightLinedUpMarey(d) {
      ...
      linedUp.appendOnce('text', 'mareyannotation start')
        ...
        .text(moment(first.time * 1000).zone(VIZ.ukData.ZONE_OFFSET).format('h:mma'));
      ...
      linedUp.appendOnce('text', 'mareyannotation end')
        ...
        .attr('y', y + 15)
        .style('text-anchor', 'middle')
        .text(moment(last.time * 1000).zone(VIZ.ukData.ZONE_OFFSET).format('h:mma'));
    ...
    function drawLabels(selection, xScale, yScale) {
      ...
      labels.append('text')
          ...
          .text(function (d) {
            return moment(d.stop.time * 1000).zone(VIZ.ukData.ZONE_OFFSET).format('h:mma');
          })
  ...
  fixedLeft.selectAll('.highlight')
    ...
    .on('mouseover', function () {
      ...
      var others = _.without(lines.map(function (d) { return d.id; }), line);
  ...
  /* 6. Day picker: load a day's trips and re-render the whole section
   *************************************************************/
  // the h4's text node that precedes the day select; swap it on day change
  var sectionTitleNode = document.getElementById('marey-day-select').parentNode.firstChild;
  function updateSectionTitle(date) {
    var formatted = moment.utc(date, 'YYYY-MM-DD').zone(VIZ.ukData.ZONE_OFFSET).format('dddd D MMMM YYYY');
    sectionTitleNode.nodeValue = 'Trips on ' + formatted + '\n          ';
  }

  // clear the diagrams and rebuild their svg scaffolding so the next render
  // starts from scratch (also forces renderMarey to re-render at the same
  // screen width and resets the zoom/freeze interaction state)
  function resetSectionSvg() {
    lastWidth = null;
    var mareyDiv = d3.select('.marey').style('text-align', 'left');
    mareyDiv.text('');
    marey = mareyDiv.append('svg');
    d3.select('.lined-up-marey').text('');
    var sideMap = fixedLeft.select('.side-map');
    sideMap.text('');
    mapGlyphSvg = sideMap.append('svg');
    marey.call(tip);
  }

  // re-render the Marey and the map glyph at the last known width
  function renderSection() {
    showingMap = true;
    renderMarey(marey, fixedRightWidth);
    renderTrainsAtTime();
  }

  // number of the most recently requested load; stale loads are discarded
  var loadToken = 0;
  function loadTripsFor(date, changesPageSize) {
    var token = ++loadToken;
    var day = _.find(days, function (d) { return d.date === date; });
    var lineIds = day ? day.lines.map(function (d) { return d.line; }) : [];
    resetSectionSvg();
    d3.selectAll('.marey, .lined-up-marey').text('Loading train data...').style('text-align', 'center');
    VIZ.requiresData(VIZ.ukData.tripFilesForDate(date, lineIds), changesPageSize)
      ...
      .done(function () {
        ...
        var rawTrips = [].concat.apply([], arguments);
        trips = VIZ.ukData.convertTrips(rawTrips);
        trips.forEach(function (d) {
          d.stops = d.stops || [];
          var m = moment(d.begin * 1000).zone(VIZ.ukData.ZONE_OFFSET);
          d.secs = m.diff(m.clone().startOf('day')) / 1000;
        });
        minUnixSeconds = d3.min(trips, function (d) { return d.begin; });
        maxUnixSeconds = d3.max(trips, function (d) { return d.end; });
        linedUpTrips = trips.filter(function (trip) {
          return _.contains(linedUpMareyStartingStations, trip.stops[0].stop);
        });
        resetScale(linedUpYScale);
        resetScale(linedUpTimeScale);
        linedUpYScale.domain([0, d3.max(linedUpTrips, function (trip) {
          return trip.stops[trip.stops.length - 1].time - trip.stops[0].time;
        })]);
        linedUpTimeScale.domain([0, d3.max(linedUpTrips, function (trip) {
        ...
        hoveredTrip = null;
        lastTime = minUnixSeconds;
        updateSectionTitle(date);
        resetSectionSvg();
  ...
  // populate the day picker and re-render when the day changes
  var daySelect = d3.select('#marey-day-select');
  ...
  if (defaultDate) {
    daySelect.property('value', defaultDate);
  }
  daySelect.on('change', function () {
    if (this.value) { loadTripsFor(this.value); }
  });
  ...
  // load the most recent day in the index and render the section
  if (defaultDate) {
    loadTripsFor(defaultDate, true);
  }
