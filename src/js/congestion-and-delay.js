 ...
 * Adapted for UK Rail Viz: data comes from data/delay.json, data/network.json,
 * and data/average-actual-delays.json via the uk-data adapter; day labels and
 * set-time link parsing are derived from the data's own dates.
 *
...
VIZ.requiresData([
...
}).done(function (delayRaw, networkRaw, averageActualDelays) {
  "use strict";
  d3.select(".interaction-all .loading").remove();

  // convert the raw UK data into the exemplar's in-memory shapes
  ...
  var delay = VIZ.ukData.convertDelay(delayRaw);
  ...
  // Per-day labels derived from the data (the synthetic week runs from
  // 2025-04-02), so the chart doesn't hardcode a calendar month.  dayDates
  // is also used to parse the set-time links' clock times (see section 7).
  var dayNames = byDay.map(function (dayBuckets) {
    return moment(dayBuckets[0].time).zone(VIZ.ukData.ZONE_OFFSET).format('ddd');
  });
  var dayLabels = byDay.map(function (dayBuckets) {
    return moment(dayBuckets[0].time).zone(VIZ.ukData.ZONE_OFFSET).format('MMM D');
  });
  var dayDates = byDay.map(function (dayBuckets) {
    return moment(dayBuckets[0].time).zone(VIZ.ukData.ZONE_OFFSET).format('YYYY-MM-DD');
  });
  ...
  dayRowLabels.append('text')
    ...
    .text(function (d) { return dayNames[d]; });
  dayRowLabels.append('text')
  ...
  function mouseoverTime(day, theTime) {
    ...
    var fullTime = moment(theTime).utc().format('h:mm a') + ' on ' + dayNames[day] + ' ' + dayLabels[day];
  ...
  // Draw a colored dot at the end of each line.  The termini come from the
  // data (first/last station of each line's path) rather than hardcoded
  // station ids, so any network configuration works.
  paths.forEach(function (path, i) {
    var color = lines[i].color;
    drawLineEndDot(path[0], color);
    drawLineEndDot(path[path.length - 1], color);
  });
  ...
  d3.selectAll('.interaction-all a.set-time')
    ...
    .on('mouseover', function () {
      ...
      // Parse the clock time against the data's own date for that day.
      // moment 2.6's 'Z' token double-applies offsets other than -0500,
      // so no zone token is used here; the diff vs startOf('day') is the
      // ms-since-midnight value the chart expects.
      var parsed = moment(dayDates[day] + ' ' + time, 'YYYY-MM-DD hh:mm a');
      var ms = parsed.diff(parsed.clone().startOf('day'));
      mouseoverTime(day, ms);
    });

  // Bootstrap the visualization, start by showing day 1 at 5:30PM
  var initalTime = moment(dayDates[1] + ' 5:30 pm', 'YYYY-MM-DD hh:mm a');
