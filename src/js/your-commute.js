 ...
 * data/commute-<origin>.json via the uk-data adapter; commute
 * data is generated per PoC commute origin (config/poc.json), so picking
 * any origin with a data file renders the scatterplot; others show a
 * friendly message instead of requesting a missing file.
 *
...
VIZ.requiresData([
...
], true).done(function (networkRaw) {
  "use strict";
  // UK adaptation: convert the network file into the exemplar's in-memory
  // shapes (network, spider, paths) via the uk-data adapter
  ...
  (function renderMap() {
    ...
    function dragStart(d) {
      ...
      // UK adaptation: only request the file if commute data exists for this origin
      if (VIZ.fileSizes['data/commute-' + draggingFrom.id + '.json']) {
        VIZ.requiresData(['json!data/commute-' + draggingFrom.id + '.json']);
      }
    ...
    // line color circles at the end of the corridor
    ...
    // color circles at the end of each line, derived from the data rather
    // than hardcoded station ids so any network configuration works
    paths.forEach(function (path) {
      dot(path[0], 'line-end');
      dot(path[path.length - 1], 'line-end');
    });
  }());
  ...
  /* 4. When a drag finishes, render the scatterplot of wait/transit times between chosen stops
   *************************************************************/
  // UK adaptation: show a friendly message when no commute data exists for
  // the requested origin (or destination), and clear any previously rendered
  // chart.  The scatterplot scaffolding is left in place so that the next
  // pair with data can re-render.
  function showNoCommuteData(from) {
    d3.select('.pick-two').classed('loading-data', false);
    d3.selectAll('.section-pick-two .loading').html('&nbsp;');
    details.text('No commute data available for ' + from + ' yet — try a London terminal like Fenchurch Street');
    dataLayer.selectAll('.circle, .highlighter, .highlighter-text').remove();
    [topRange, topUnder, topMedian, bottomRange, bottomUnder, bottomMedian].forEach(function (path) {
      path.attr('d', null);
    });
    title.text('');
    subtitle.text('');
    sideKey.style('opacity', 0);
    zeroMinutesLine.style('opacity', 0);
    d3.select('.pick-two .key').text('');
  }
  ...
  function render(from, to) {
    ...
    // UK adaptation: the loader throws synchronously for files it has no
    // size/hash for, so check before requesting commute data for this origin
    if (!VIZ.fileSizes['data/commute-' + from + '.json']) {
      showNoCommuteData(from);
      return;
    }
    VIZ.requiresData([
      'json!data/commute-' + from + '.json'
      ...
      d3.select('.pick-two').classed('loading-data', false);
      d3.selectAll(".section-pick-two .loading").html('&nbsp;');
      // UK adaptation: the origin's file loaded but has no data for this
      // destination, so treat it like a missing origin
      if (!weekdayRollup[to]) {
        showNoCommuteData(from);
        return;
      }
      // clear any "no commute data" message left over from a previous pick
      details.text('');
   ...
   * #your-commute.FST.SOS
   * it loads data into the scatterplot from Fenchurch Street to
   * Southend Central
   *************************************************************/
  ...
  if (hashData && idToNode[hashData[0]] && idToNode[hashData[1]]) {
    ...
    // if no specific station pair is selected, then start by showing a
    // c2c hop from Fenchurch Street to Southend Central
    render('FST', 'SOS');
  }
  ...
  d3.selectAll('.section-pick-two a.pick-two-choose')
    ...
    .on('mouseover', function () {
      ...
      // UK adaptation: only request the file if commute data exists for this origin
      var start = d3.select(this).attr('data-start');
      if (VIZ.fileSizes['data/commute-' + start + '.json']) {
        VIZ.requiresData(['json!data/commute-' + start + '.json']);
      }
    });
