 ...
 * data/marey-index.json via the uk-data adapter; time range is derived
 * from the trips; station terminal dots are computed from the network.
 *
...
(function () {
  ...
  VIZ.requiresData([
    // nodes and links that comprise the rail network
    'json!data/network.json',
    // day/line index used to find the marey trip files
    'json!data/marey-index.json'
  ], true).done(function (networkRaw, mareyIndex) {
    ...
    // Now load the marey trip data for the most recent day in the index
    // and start the animation.  1 minute = 1 second.
    var days = mareyIndex.days || [];
    var latest = days[days.length - 1];
    var date = latest && latest.date;
    var lineIds = latest && latest.lines ? latest.lines.map(function (d) { return d.line; }) : [];
    var minUnixSeconds, maxUnixSeconds;
    VIZ.requiresData(VIZ.ukData.tripFilesForDate(date, lineIds)).done(function () {
    ...
    function renderTrainsAtTime(unixSeconds, now) {
      ...
      if (unixSeconds) { svg.select('.time-display').text(function () {
        var t = moment(unixSeconds * 1000).zone(VIZ.ukData.ZONE_OFFSET);
        return t.format('ddd D/M h:mm a');
      }); }
    }
