 ...
 * Adapted for UK Rail Viz: data comes from data/station-frequency.json,
 * data/station-usage.json and data/network.json via the uk-data adapter;
 * station annotations, line lists and map-key sizes are derived from the
 * data instead of hardcoded MBTA values.
 *
...
VIZ.requiresData([
  'json!data/station-frequency.json',
  'json!data/station-usage.json',
...
}).done(function (frequency, usage, networkRaw) {
  "use strict";
  ...
  // Convert the UK rail data into the turnstile-heatmap, stop-mapping and
  // network + spider shapes the exemplar's rendering code reads (see uk-data.js)
  var turnstile = VIZ.ukData.convertTurnstile(frequency, usage);
  var turnstileToGtfs = VIZ.ukData.turnstileToStationIds(frequency);
  var convertedNetwork = VIZ.ukData.convertNetwork(networkRaw);
  var network = convertedNetwork.network;
  var spider = convertedNetwork.spider;
  ...
  // Annotations to display below turnstile heatmaps, derived from the data
  // so that no station names are hardcoded (the exemplar hand-wrote these
  // for a handful of MBTA stations).  Generated for the busiest, second- and
  // third-busiest, and least-busy stations, using each station's own average
  // weekday pattern for the peak-hour text.  stationDetails[name] -> text,
  // consumed by updateShownStations() below.
  var stationDetails = {};

  // average per-hour weekday pattern for one direction of travel, skipping
  // the holiday cell (week 2 Monday) that the data treats as offpeak
  function weekdayPattern(stop, dir) {
    var sums = [], counts = [];
    for (var h = 0; h < 24; h++) { sums.push(0); counts.push(0); }
    stop.times.forEach(function (t) {
      if (t.day >= 1 && t.day <= 5 && !(t.week === 2 && t.day === 1)) {
        sums[t.hour] += t[dir];
        counts[t.hour]++;
      }
    });
    return d3.range(24).map(function (h) { return counts[h] ? sums[h] / counts[h] : 0; });
  }

  function peakHour(pattern) {
    var hour = 0, value = -1;
    pattern.forEach(function (v, h) { if (v > value) { value = v; hour = h; } });
    return hour;
  }
      ...
      svg
        .onOnce('mouseover', 'rect', function (d) {
          ...
          if (!isAllStationHeatmap) {
            ...
            var day = moment(d.time).zone(VIZ.ukData.ZONE_OFFSET).format('ddd MMM D');
            drawMapKey(perMinuteKeySizes, perHourPerMinuteSizeScale, type + ' on ' + day + ' from ' + time + ' to ' + timePlusOne, 'per minute');
          }
        })
  ...
  var perHourPerMinuteSizeScale = d3.scale.sqrt()
      ...
      .range([2, 7]);
  // Representative sizes for the map key.  The exemplar hardcodes values
  // tuned to the MBTA data ([500, 10000, 19400] entries per day, and
  // [0, 40, 82] per minute); derive the equivalent samples from the
  // converted data so the key matches what the map actually draws.
  var averageEntriesPerDay = d3.values(averageSecondsBetweenStops);
  var averageEntriesPerDayExtent = d3.extent(averageEntriesPerDay);
  var averageEntriesPerDayMean = d3.mean(averageEntriesPerDay);
  var perDayKeySizes = [averageEntriesPerDayExtent[0], averageEntriesPerDayMean, averageEntriesPerDayExtent[1]];
  var perMinuteKeySizes = [0, turnstile.max / 120, turnstile.max / 60];
  ...
  function drawMap(mapGlyphContainer) {
    ...
    // The exemplar hardcodes the MBTA terminals; derive the UK terminals
    // from each line's ordered stop list (first and last stop of the line)
    convertedNetwork.paths.forEach(function (path, lineIndex) {
      dot(path[0], convertedNetwork.lines[lineIndex].id);
      if (path.length > 1) {
        dot(path[path.length - 1], convertedNetwork.lines[lineIndex].id);
      }
    });
  }
  ...
  function resetMapKey() {
    drawMapKey(perDayKeySizes, sizeScale, 'entries on average day', 'per day');
    d3.selectAll('.section-people .glyph circle')
      .attr('r', function (d) { return sizeScale(averageSecondsBetweenStops[d.id]); });
  }
  ...
  if (!VIZ.ios) {
    stationTable.onOnce('mouseover', '.heatmap rect', function (d) {
      ...
      d3.selectAll('.section-people .glyph circle')
        .attr('r', function (d) { return perHourSizeScale(updatedSizes[d.id]); });
      drawMapKey(perMinuteKeySizes, perHourPerMinuteSizeScale, d.type + ' on ' + (d.day === 'weekday' ? 'weekdays' : 'weekends/holidays') + ' from ' + time + ' to ' + timePlusOne, 'per minute');
    })
  ...
  function highlightStationOrStationRange(d) {
    ...
    if (dragOrigin) {
      ...
      // the exemplar selects by the MBTA station-id class; ours selects the
      // same elements (glyph station circles and table rows) generically
      container.selectAll('.glyph circle, .turnstile-table .row').classed('active', isActive);
    } else {
  ...
  function getTextForRollover(d, dataType) {
    return [
      [
        moment(d.time).zone(VIZ.ukData.ZONE_OFFSET).format('ddd MMM D [from] ha'),
        'to',
        moment(d.time).zone(VIZ.ukData.ZONE_OFFSET).add(1, 'hour').format('ha')
      ].join(' '),
      [
